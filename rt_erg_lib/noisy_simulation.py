...
class noisy_simulation():
    def __init__(self, size, noise, t_dist, model, erg_ctrl, true_env, belief_env, tf):
        self.size = size
        self.noise = noise
    ...
    def start(self, report=False):
        self.log = {'truth': [], 'belief': []}
        init_state = np.array([3., 3., 0.2, 0.2])
        belief_state = self.belief_env.reset(init_state)
        true_state = self.true_env.reset(init_state)
        for t in tqdm(range(self.tf)):
            start_time = time.time()
            ctrl = self.erg_ctrl(belief_state)
            noisy_ctrl = ctrl + self.noise * np.random.randn(2)
            belief_state = self.belief_env.step(ctrl)
            true_state = self.true_env.step(noisy_ctrl)
